...
do
    ...
    mkdir -p users/${names[$i]}
    cd users/${names[$i]}
    ln -s ../../terraform/* .
    ln -s ../../terraform/.t* .
    terraform init &&
    terraform apply -auto-approve -var email=${emails[$i]} -var name=${names[$i]}  -var zone=${zones[$(($i % ${#zones[@]}))]}  &> ../../logs/logs-apply/${names[$i]}.log &
    cd ../.. 
    clear
    echo Creating cluster number $(($i+1))
    sleep 7
done

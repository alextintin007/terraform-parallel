...
do
    ...
    cd users/${names[$i]}
    terraform destroy -auto-approve -var email=${emails[$i]} -var name=${names[$i]}  -var zone=${zones[$(($i % ${#zones[@]}))]}  &> ../../logs/logs-destroy/${names[$i]}.log &
    cd ../.. 
    clear
    echo Destroying cluster number $(($i+1))
    sleep 7
done
